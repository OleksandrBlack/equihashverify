...
int Equihash<N,K>::InitialiseState(eh_HashState& base_state, bool safe_salt)
{
                               ...
                               const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    LogPrint(SAFELog::POW, "Generating first list\n");
    ...
    for (unsigned int r = 1; r < K && X.size() > 0; r++) {
        LogPrint(SAFELog::POW, "Round %u:\n", r);
        // 2a) Sort the list
        LogPrint(SAFELog::POW, "- Sorting list\n");
        std::sort(X.begin(), X.end(), CompareSR(CollisionByteLength));
        if (cancelled(ListSorting)) throw solver_cancelled;

        LogPrint(SAFELog::POW, "- Finding collisions\n");
        size_t i = 0;
        size_t posFree = 0;
    ...
    LogPrint(SAFELog::POW, "Final round:\n");
    if (X.size() > 1) {
        LogPrint(SAFELog::POW, "- Sorting list\n");
        ...
        LogPrint(SAFELog::POW, "- List is empty\n");
                                   ...
                                   const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    LogPrint(SAFELog::POW, "Found %d partial solutions\n", partialSolns.size());

    // Now for each solution run the algorithm again to recreate the indices
    LogPrint(SAFELog::POW, "Culling solutions\n");
...
invalidsolution:
    ...
    LogPrint(SAFELog::POW, "- Number of invalid solutions found: %zu\n", invalidCount);
...
bool Equihash<N,K>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln)
{
    if (soln.size() != SolutionWidth) {
        LogPrint(SAFELog::POW, "Invalid solution length: %d (expected %d)\n",
    ...
    while (X.size() > 1) {
        ...
        for (size_t i = 0; i < X.size(); i += 2) {
            if (!HasCollision(X[i], X[i+1], CollisionByteLength)) {
                LogPrint(SAFELog::POW, "Invalid solution: invalid collision length between StepRows\n");
                LogPrint(SAFELog::POW, "X[i]   = %s\n", X[i].GetHex(hashLen));
                LogPrint(SAFELog::POW, "X[i+1] = %s\n", X[i+1].GetHex(hashLen));
                return false;
            }
            if (X[i+1].IndicesBefore(X[i], hashLen, lenIndices)) {
                LogPrint(SAFELog::POW, "Invalid solution: Index tree incorrectly ordered\n");
                return false;
            }
            if (!DistinctIndices(X[i], X[i+1], hashLen, lenIndices)) {
                LogPrint(SAFELog::POW, "Invalid solution: duplicate indices\n");
                return false;
            }
...
template int Equihash<96,3>::InitialiseState(eh_HashState& base_state, bool safe_salt);
#ifdef ENABLE_MINING
...
template int Equihash<200,9>::InitialiseState(eh_HashState& base_state, bool safe_salt);
#ifdef ENABLE_MINING
...
template int Equihash<144,5>::InitialiseState(eh_HashState& base_state, bool safe_salt);
#ifdef ENABLE_MINING
...
template int Equihash<96,5>::InitialiseState(eh_HashState& base_state, bool safe_salt);
#ifdef ENABLE_MINING
...
template int Equihash<48,5>::InitialiseState(eh_HashState& base_state, bool safe_salt);
#ifdef ENABLE_MINING
